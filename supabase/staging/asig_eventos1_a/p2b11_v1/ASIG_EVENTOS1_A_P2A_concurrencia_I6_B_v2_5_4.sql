-- =====================================================================
-- ASIG.EVENTOS.1-A · P2-A · CONCURRENCIA I6 — SESIÓN B (v2.5.4)
-- NO EJECUTAR HASTA P2-B1. Dos fases con BARRERA EFECTIVA (\prompt). Correr con I6_A.
...
--
-- Solicitud IDÉNTICA a I6_A (mismo asig_i6, opreq_i6, motivo):
--   psql -X -v ON_ERROR_STOP=1 -v admin_uid='UUID' -v asig_i6='ID' -v opreq_i6='MISMO-UUID-que-A' -f <este archivo>
-- =====================================================================
\set ON_ERROR_STOP on
\if :{?admin_uid}
\else
  \warn '>>> FALTA parámetro externo: -v admin_uid'
  DO $abort$ BEGIN RAISE EXCEPTION 'I6_B: FALTA -v admin_uid (error SQL real; exit<>0 con ON_ERROR_STOP)'; END $abort$;
\endif
\if :{?asig_i6}
\else
  \warn '>>> FALTA parámetro externo: -v asig_i6'
  DO $abort$ BEGIN RAISE EXCEPTION 'I6_B: FALTA -v asig_i6 (error SQL real; exit<>0 con ON_ERROR_STOP)'; END $abort$;
\endif
\if :{?opreq_i6}
\else
  \warn '>>> FALTA parámetro externo: -v opreq_i6'
  DO $abort$ BEGIN RAISE EXCEPTION 'I6_B: FALTA -v opreq_i6 (error SQL real; exit<>0 con ON_ERROR_STOP)'; END $abort$;
\endif
...
BEGIN
  ...
  ASSERT current_user = 'authenticated', 'I6_B: la RPC no corre como authenticated';
  ASSERT auth.uid() IS NOT NULL, 'I6_B: el claim local no persistió';
  ASSERT v.fecha IS NOT NULL, 'I6_B: fecha objetivo no calculada (¿asig_i6 válida?)';
  v_tok := (public.preparar_retiro_asignacion_individual(v.asig))->>'token_anti_stale';
  UPDATE p SET tok = v_tok;
  RAISE NOTICE 'I6_B FASE1 lista.';
END $$;
RESET ROLE;

-- ===== BARRERA EFECTIVA =====
\prompt '>>> BARRERA I6_B: con ambas FASE 1 hechas, presione ENTER para ejecutar la FASE 2 ' _barrera
...
BEGIN
  ...
  ASSERT (r->>'resultado') IN ('created','replayed')
         ...
         'I6_B: resultado inesperado -> '||r::text;
  RAISE NOTICE 'I6_B OK: resultado=%', r->>'resultado';
END $$;
RESET ROLE;
...
-- Esperado: EXACTAMENTE 1 evento — 1 created + 1 replayed; idempotencia por opreq
...
SELECT set_config('asig_conc.asig',  :asig_i6::text, false);
DO $verif$
DECLARE n int;
BEGIN
  ...
  RESET ROLE;
  IF n <> 1 THEN
    RAISE EXCEPTION 'I6 VERIFICACIÓN FALLIDA: asignación %: se esperaba exactamente 1 evento y hay % (el replay no debe duplicar).',
                    current_setting('asig_conc.asig'), n;
  END IF;
  RAISE NOTICE '>>> I6 VERIFICACIÓN OK: exactamente 1 evento (1 created + 1 replayed; idempotencia por opreq).';
END $verif$;
