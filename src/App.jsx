...
function FechaInput({value,onChange,style,max}){
  ...
  return (
    <div>
      <div>
        <input style={{...(style||INP),width:'100%'}} value={txt} onChange={onText} onBlur={onBlurText} placeholder="dd/mm/aaaa" inputMode="numeric" maxLength={10}/>
      </div>
...
function Trabajadores({data,insert,update,saveAsignacion,terminarAsignacion,contratoId,reload}){
  ...
  const openNuevaAsignacion=()=>{
    ...
    const tieneActivaSinBase=asignacionesTrabActivas.length>0&&!tieneBaseInicialActiva;
    if(tieneActivaSinBase){
      alert("Este trabajador tiene asignaciones activas, pero aún no tiene una asignación inicial definida.\nPara evitar clasificarlo incorrectamente, este caso debe revisarse antes de crear una nueva asignación operativa.");
      return;
    }
    ...
    if(esBaseInicial){
      ...
      setAsigForm({trabajador_id:form.id,contrato_id:contratoId||"",activo:true,estado_asig:"activa",afecta_remuneracion:true,
        ...
        gratificacion_metodo_asig:form.metodo_gratificacion||null,gratificacion_porcentaje_asig:"",gratificacion_observacion_asig:"",
  ...
  const guardarAsignacion=async(overrideDocumental=null)=>{
    ...
    // INTEG.ASIG-ANEXO-COSTO.1-A · Incremento 3A microfix: guarda defensiva final.
    // Una asignación base_inicial SIEMPRE persiste remuneracional, sin modalidad de cobertura,
    // con los montos de la ficha del trabajador — sin importar qué haya quedado en el formulario.
    if(registro.rol_asignacion==="base_inicial"){
      registro.afecta_remuneracion=true;
      registro.modalidad_cobertura=null;
      registro.sueldo_asignado=Number(form.sueldo_base||0);
      registro.bono_asistencia=Number(form.bono_asistencia||0);
      registro.bono_movilizacion=Number(form.bono_movilizacion||0);
      registro.bono_colacion=Number(form.bono_colacion||0);
      registro.gratificacion_metodo_asig=form.metodo_gratificacion||null;
      registro.gratificacion_monto=Number(form.gratificacion_monto||0);
              ...
              :<>
                ...
                {asigForm&&(<div style={{background:C.surfaceAlt,border:`1px solid ${C.border}`,borderRadius:8,padding:14,marginBottom:14}}>
                  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
                    ...
                    <FL label="Tipo de asignación"><select disabled={asigForm.rol_asignacion==="base_inicial"} style={{...INP,...(asigForm.rol_asignacion==="base_inicial"?{background:'#f3f4f6',cursor:'not-allowed',color:C.textMuted}:{})}} value={asigForm.afecta_remuneracion===false?"no":"si"} onChange={e=>setAsigForm({...asigForm,afecta_remuneracion:e.target.value==="si"})}><option value="si">💰 Remuneracional: suma a liquidación</option><option value="no">👁 Operacional: supervisión/control, no suma</option></select></FL>
    ...
    <div>
      ...
      {form&&(
        <FormCard onSave={save} onCancel={()=>setForm(null)} saveLabel="Registrar" accent={C.red}>
          ...
          <FL label="Trabajador"><select style={INP} value={form.trabajador_id} onChange={e=>setForm({...form,trabajador_id:e.target.value})}><option value="">— Sin asignar —</option>{/* RRHH.2-B FASE 4-F2: trabajador opcional; solo operativos, sin desvinculados/PREINGRESO. */}{data.trabajadores.filter(t=>puedeOperarEnMotor(t)).map(t=><option key={t.id} value={t.id}>{t.nombre}</option>)}</select></FL>
